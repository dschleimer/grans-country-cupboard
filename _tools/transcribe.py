#!/usr/bin/env python3
"""Transcription progress tracker for Gran's Country Cupboard recipes.

Tracks progress per recipe file. Pages are the batching unit (which image
to read), but completion is per-file.

Usage:
  python _tools/transcribe.py status               # show progress
  python _tools/transcribe.py next-batch [-n N]    # next N pages of recipes
  python _tools/transcribe.py mark-done <files>    # record files as completed
  python _tools/transcribe.py skip <file> <reason> # skip a recipe with reason
  python _tools/transcribe.py skipped              # list all skipped recipes
  python _tools/transcribe.py reset                # clear all progress
"""
...
PROGRESS_FILE = Path(".recipe_stats/transcribe_progress.json")
...
def load_progress() -> dict:
    if PROGRESS_FILE.exists():
        ...
        # Ensure skipped key exists (forward compat)
...
def get_all_untranscribed() -> list[str]:
    """Return sorted list of recipe files that have Needs Transcription."""
...
def cmd_status(args) -> None:
    ...
    all_files = get_all_untranscribed()
    ...
    total = len(all_files) + len(done)  # done files no longer have the tag
    n_done = len(done)
    n_skipped = len(skipped)
    pct = (100 * n_done // total) if total else 0

    print(f"Transcription progress: {n_done}/{total} ({pct}%)")
...
def cmd_mark_done(args) -> None:
    ...
    if newly_done:
        print(f"Marked {len(newly_done)} recipe(s) as done.")
    else:
...
def main() -> None:
    ...
    md = sub.add_parser("mark-done", help="Record recipe files as completed")
