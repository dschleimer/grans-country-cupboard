...
MAPPINGS_PATH = REPO_ROOT / "_data" / "ingredient_mappings.yml"
...
def resolve_ingredient(raw, mappings, canonical_set):
    ...
    # 3. Try iterative modifier prefix stripping
    ...
    while True:
        ...
        for mod in MODIFIER_PREFIXES:
            mod_lower = mod.lower()
            if remainder_key.startswith(mod_lower + " "):
                remainder_norm = remainder_norm[len(mod) + 1:].strip()
                remainder_key = remainder_norm.lower()
...
def bootstrap(recipe_dir):
    ...
    with open(draft_path, "w", encoding="utf-8") as f:
        ...
        f.write("# Review and move to _data/ingredient_mappings.yml\n")
        f.write("#\n")
